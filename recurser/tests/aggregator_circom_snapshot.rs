...
fn aggregator_renders_required_layout() {
    ...
    let out = gen_aggregator(0, "zisk_final.verifier.circom", &zisk_vk, &program_vks, &stark, &templates).unwrap();
    ...
    // Verifier mux.
    assert!(out.contains("vA.rootC <== MultiMux1(4)([programVK_A, rootCVadcopFinalZisk], isRegisteredProgramA);"));
    assert!(out.contains("vB.rootC <== MultiMux1(4)([programVK_B, rootCVadcopFinalZisk], isRegisteredProgramB);"));

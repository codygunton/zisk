...
case ${1:-patched} in
  patched)
    cases=(illegal:34 ebreak:35 load_fault:37 store_fault:39 bad_fetch:33 clean_exit:0)
    ;;
  baseline)
    # The pre-change executable silently succeeds on transpile-time halts and panics for memory
    # faults. A fetch into a gap runs until the explicit step cap and returns a generic error.
    cases=(illegal:0 ebreak:0 load_fault:101 store_fault:101 bad_fetch:1 clean_exit:0)
    ;;
  *)

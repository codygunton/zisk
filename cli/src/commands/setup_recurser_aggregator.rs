use std::path::PathBuf;
use std::sync::Arc;

use anyhow::{bail, Context, Result};
use fields::Goldilocks;
use proofman_common::{MpiCtx, ProofCtx};
use recurser::setup::{run_setup_recurser_aggregator, SetupRecurserAggregatorOptions};
use rom_setup::rom_merkle_setup;
use zisk_build::ZISK_VERSION_MESSAGE;
use zisk_prover_backend::{setup_logger, GuestProgram};

use crate::common::{get_home_zisk_path, get_proving_key};


#[derive(clap::Args)]
#[command(author, about, long_about = None, version = ZISK_VERSION_MESSAGE)]
/// Generate the recurser-aggregator setup on top of an existing vadcop_final
/// build directory. The program-VK allowlist is derived directly from the
/// provided guest ELFs via `rom_merkle_setup`. See
/// recurser/docs/aggregator-flow.md §12.
pub struct ZiskSetupRecurserAggregator {
    /// ZisK setup directory to read from. Must contain
    /// `provingKey/pilout.globalInfo.json` and
    /// `provingKey/<name>/vadcop_final/`. Defaults to `~/.zisk`.
    #[arg(short = 's', long = "setup-dir")]
    pub setup_dir: Option<String>,

    /// Where to write the generated recurser-aggregator artifacts. Must differ
    /// from `--setup-dir`. Defaults to `./build`.
    #[arg(short = 'o', long = "output-dir", default_value = "build")]
    pub output_dir: String,

    /// Identifier for this recurser setup. Artifacts land under
    /// `<output-dir>/provingKey/recurser/<recurser-id>/`. Omit to use a
    /// random hex placeholder — the identification scheme is still TBD.
    #[arg(long = "recurser-id")]
    pub recurser_id: Option<String>,

    /// Guest program ELFs to register as recurser leaves. Each ELF's program
    /// VK is derived (or read from cache) via `rom_merkle_setup` and baked
    /// into the aggregator's allowlist. Order is significant — it fixes the
    /// `programVKs[]` index of each program for the lifetime of the setup.
    #[arg(short = 'e', long = "program-elf", num_args = 1.., required = true)]
    pub program_elfs: Vec<PathBuf>,

    /// User-supplied `AggregatePublics` Circom body (required).
    #[arg(long = "aggregate-publics-template")]
    pub aggregate_publics_template: String,

    /// User-supplied `PreparePublics` Circom body. Omit for the built-in
    /// identity default.
    #[arg(long = "prepare-publics-template")]
    pub prepare_publics_template: Option<String>,

    /// User-supplied `CheckPublics` Circom body. Omit for the built-in no-op
    /// default.
    #[arg(long = "check-publics-template")]
    pub check_publics_template: Option<String>,

    /// Number of side inputs threaded into the three sub-templates.
    #[arg(long = "n-private-inputs", default_value_t = 0)]
    pub n_private_inputs: usize,

    /// Path to a precomputed proving key (defaults to the standard ZisK
    /// location). Needed to construct the `ProofCtx` that `rom_merkle_setup`
    /// runs against.
    #[arg(short = 'k', long = "proving-key")]
    pub proving_key: Option<PathBuf>,

    /// Cache directory for the rom-setup verkey/bin artifacts. Defaults to
    /// `~/.zisk/cache` (rom-setup's own convention).
    #[arg(long = "cache-dir")]
    pub cache_dir: Option<PathBuf>,

    /// Verbosity (-v, -vv)
    #[arg(short = 'v', long, action = clap::ArgAction::Count)]
    pub verbose: u8,
}

impl ZiskSetupRecurserAggregator {
    pub fn run(&self) -> Result<()> {
        setup_logger(self.verbose.into());

        rayon::ThreadPoolBuilder::new().stack_size(64 * 1024 * 1024).build_global().ok();

        let program_vks = self.derive_program_vks()?;

        let setup_dir = match &self.setup_dir {
            Some(p) => p.clone(),
            None => get_home_zisk_path()?
                .to_str()
                .context("default ~/.zisk path is not valid UTF-8")?
                .to_string(),
        };

        let opts = SetupRecurserAggregatorOptions {
            setup_dir,
            output_dir: self.output_dir.clone(),
            recurser_id: self.recurser_id.clone(),
            program_vks,
            n_private_inputs: self.n_private_inputs,
            prepare_publics_template: self.prepare_publics_template.clone(),
            check_publics_template: self.check_publics_template.clone(),
            aggregate_publics_template: self.aggregate_publics_template.clone(),
        };

        run_setup_recurser_aggregator(&opts)
    }

    fn derive_program_vks(&self) -> Result<Vec<[String; 4]>> {
        let proving_key = get_proving_key(self.proving_key.as_ref())?;
        let mpi_ctx = Arc::new(MpiCtx::new());
        // CPU-only: rom_merkle_setup doesn't need device buffers, and we want
        // this command to be cheap to invoke for VK derivation alone.
        let pctx = ProofCtx::<Goldilocks>::create_ctx(
            proving_key,
            false,
            self.verbose.into(),
            mpi_ctx,
            false,
        )?;

        let mut vks: Vec<[String; 4]> = Vec::with_capacity(self.program_elfs.len());
        for elf_path in &self.program_elfs {
            tracing::info!("Deriving program VK from ELF: {}", elf_path.display());
            let guest_program = GuestProgram::from_uri(
                elf_path
                    .to_str()
                    .with_context(|| format!("Non-UTF-8 ELF path: {}", elf_path.display()))?,
            )?;
            let program_vk = rom_merkle_setup::<Goldilocks>(&pctx, guest_program.elf(), &self.cache_dir)
                .with_context(|| format!("rom_merkle_setup failed for {}", elf_path.display()))?;
            let limbs = vk_bytes_to_limbs(&program_vk.vk).with_context(|| {
                format!("VK from {} did not decode into 4 u64 limbs", elf_path.display())
            })?;
            // Duplicate VKs in the registry would break the membership check's
            // soundness (the product-of-(1-eq) formulation assumes uniqueness).
            // Enforce here so the circuit doesn't have to.
            if let Some(prior_idx) = vks.iter().position(|existing| existing == &limbs) {
                bail!(
                    "Duplicate program VK at --program-elf #{} ({}); already registered at #{} ({})",
                    vks.len(),
                    elf_path.display(),
                    prior_idx,
                    self.program_elfs[prior_idx].display(),
                );
            }
            vks.push(limbs);
        }
        Ok(vks)
    }
}

fn vk_bytes_to_limbs(vk: &[u8]) -> Result<[String; 4]> {
    if vk.len() != 32 {
        bail!("expected 32 bytes (4 × u64 LE), got {}", vk.len());
    }
    let mut limbs: [String; 4] = Default::default();
    for i in 0..4 {
        let bytes: [u8; 8] = vk[i * 8..(i + 1) * 8].try_into().unwrap();
        limbs[i] = u64::from_le_bytes(bytes).to_string();
    }
    Ok(limbs)
}

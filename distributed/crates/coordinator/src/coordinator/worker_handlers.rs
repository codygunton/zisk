...
impl Coordinator {
    ...
    pub(crate) async fn handle_stream_setup_program_ack(
        ...
        if let Some((vks, hash_id, program_name, with_hints)) = outcome {
            self.finalize_setup(&job_id, vks, hash_id, program_name, with_hints).await;
        ...
        let event = match validate_setup_vks(job_id.as_str(), vks) {
            Ok(vk) => {
                self.active_setups.write().await.insert(
                    SetupKey::new(hash_id, with_hints),
    ...
    async fn prune_setup_pending_for_lost_worker(&self, worker_id: &WorkerId) {
        ...
        for (job_id, vks, hash_id, program_name, with_hints) in completions {
            info!(
